...
J9::X86::TreeEvaluator::inlineStringLatin1Inflate(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::Node *destOffNode = node->getChild(3);

   TR::Register *srcBufferReg = cg->evaluate(node->getChild(0));
   TR::Register *initSrcOffReg = cg->evaluate(srcOffNode);
   TR::Register *srcOffsetReg = cg->gprClobberEvaluate(srcOffNode, OMR::InstOpCode::MOV4RegReg);
   TR::Register *destBufferReg = cg->evaluate(node->getChild(2));
   TR::Register *destOffsetReg = cg->gprClobberEvaluate(destOffNode, OMR::InstOpCode::MOV4RegReg);
   ...
   generateRegImmInstruction(OMR::InstOpCode::CMP4RegImm4, node, lengthReg, 0, cg);
   generateLabelInstruction(OMR::InstOpCode::JE4, node, endLabel, cg);

   // make sure the register is zero before interleaving
   generateRegRegInstruction(OMR::InstOpCode::PXORRegReg, node, zeroReg, zeroReg, cg);

   if (!destOffNode->isConstZeroValue())
      {
      // dest offset measured in characters, convert it to bytes
      generateRegRegInstruction(OMR::InstOpCode::ADD4RegReg, node, destOffsetReg, destOffsetReg, cg);
      }
   ...
   generateRegMemInstruction(OMR::InstOpCode::LEA4RegMem, node, scratchReg, generateX86MemoryReference(lengthReg, srcOffsetReg, 0, -vectorLengthConst, cg), cg);

   generateLabelInstruction(OMR::InstOpCode::label, node, startLoopLabel, cg);
   generateRegRegInstruction(OMR::InstOpCode::CMP4RegReg, node, srcOffsetReg, scratchReg, cg);
   generateLabelInstruction(OMR::InstOpCode::JG4, node, endLoopLabel, cg);

   // read 16 bytes into xmmr register
   generateRegMemInstruction(OMR::InstOpCode::MOVDQURegMem, node, xmmHighReg, generateX86MemoryReference(srcBufferReg, srcOffsetReg, 0, headerOffsetConst, cg), cg);
   // TODO; avoid this copy in the future by adding support for VPUNPCK instructions
   generateRegRegInstruction(OMR::InstOpCode::MOVDQURegReg, node, xmmLowReg, xmmHighReg, cg);
   // interleave the low order bytes with zeros and write to the destination buffer
   generateRegRegInstruction(OMR::InstOpCode::PUNPCKHBWRegReg, node, xmmLowReg, zeroReg, cg);
   generateMemRegInstruction(OMR::InstOpCode::MOVDQUMemReg, node, generateX86MemoryReference(destBufferReg, destOffsetReg, 0, headerOffsetConst + vectorLengthConst, cg), xmmLowReg, cg);

   // interleave the high order bytes with zeros and write to the destination buffer
   generateRegRegInstruction(OMR::InstOpCode::PUNPCKLBWRegReg, node, xmmHighReg, zeroReg, cg);
   generateMemRegInstruction(OMR::InstOpCode::MOVDQUMemReg, node, generateX86MemoryReference(destBufferReg, destOffsetReg, 0, headerOffsetConst, cg), xmmHighReg, cg);

   // increase src offset by size of imm register
   // increase dest offset by double, to account for the byte->char inflation
   generateRegImmInstruction(OMR::InstOpCode::ADD4RegImm4, node, srcOffsetReg, vectorLengthConst, cg);
   generateRegImmInstruction(OMR::InstOpCode::ADD4RegImm4, node, destOffsetReg, 2 * vectorLengthConst, cg);

   //   LOOP BACK
   generateLabelInstruction(OMR::InstOpCode::JMP4, node, startLoopLabel, cg);
   generateLabelInstruction(OMR::InstOpCode::label, node, endLoopLabel, cg);
   ...
   TR::InstOpCode::Mnemonic loadInstructions[] = {OMR::InstOpCode::MOVQRegMem, OMR::InstOpCode::MOVDRegMem, OMR::InstOpCode::MOVZXReg2Mem1, OMR::InstOpCode::MOVZXReg2Mem1};
   TR::InstOpCode::Mnemonic storeInstructions[] = {OMR::InstOpCode::MOVDQUMemReg, OMR::InstOpCode::MOVQMemReg, OMR::InstOpCode::S2MemReg, OMR::InstOpCode::S2MemReg};
   uint8_t readIncrements[] = {8, 4, 2, 1};

   if (!srcOffNode->isConstZeroValue())
      {
      generateRegRegInstruction(OMR::InstOpCode::ADD4RegReg, node, lengthReg, initSrcOffReg, cg);
      }
   ...
   for (int i = 0; i < sizeof(readIncrements); i++)
      ...
      generateLabelInstruction(OMR::InstOpCode::label, node, beforeLabel, cg);

      // Check if readIncrements[i] bytes remain
      generateRegMemInstruction(OMR::InstOpCode::LEA4RegMem, node, scratchReg, generateX86MemoryReference(lengthReg, - readIncrements[i], cg), cg);
      generateRegRegInstruction(OMR::InstOpCode::CMP4RegReg, node, srcOffsetReg, scratchReg, cg);
      generateLabelInstruction(OMR::InstOpCode::JG4, node, afterLabel, cg);
      ...
      if (readIncrements[i] != 2)
         ...
         if (loadInstructions[i] != OMR::InstOpCode::MOVZXReg2Mem1) // no need to interleave if zero extended
            {
            generateRegRegInstruction(OMR::InstOpCode::PUNPCKLBWRegReg, node, reg, zeroReg, cg);
            }
      ...
      if (readIncrements[i] != 1)
         ...
         generateRegImmInstruction(OMR::InstOpCode::ADD4RegImm4, node, srcOffsetReg, readIncrements[i], cg);
         generateRegImmInstruction(OMR::InstOpCode::ADD4RegImm4, node, destOffsetReg, readIncrements[i] * 2, cg);
         }

      generateLabelInstruction(OMR::InstOpCode::label, node, afterLabel, cg);
      }

   generateLabelInstruction(OMR::InstOpCode::label, node, endLabel, deps, cg);

   if (!srcOffNode->isConstZeroValue())
      {
      generateRegRegInstruction(OMR::InstOpCode::SUB4RegReg, node, lengthReg, initSrcOffReg, cg);
      }
